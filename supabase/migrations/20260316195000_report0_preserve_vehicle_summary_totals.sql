...
with repaired as (
  select
    id,
    case
      when source_summary_report_id is not null
        and coalesce((last_summary_generated_at at time zone 'utc')::date, date '1900-01-01') = (now() at time zone 'utc')::date
        then nullif(source_summary_payload ->> 'distance', '')::numeric
      else null
    end as total_distance,
    case
      when source_summary_report_id is not null
        and coalesce((last_summary_generated_at at time zone 'utc')::date, date '1900-01-01') = (now() at time zone 'utc')::date
        then nullif(source_summary_payload ->> 'hrs', '')::numeric
      else null
    end as total_engine_hours,
    case
      when source_summary_report_id is not null
        and coalesce((last_summary_generated_at at time zone 'utc')::date, date '1900-01-01') = (now() at time zone 'utc')::date
        then nullif(source_summary_payload ->> 'fuel_used', '')::numeric
      else null
    end as total_fuel_used,
    case
      when source_summary_report_id is not null
        and coalesce((last_summary_generated_at at time zone 'utc')::date, date '1900-01-01') = (now() at time zone 'utc')::date
        then nullif(regexp_replace(coalesce(source_summary_payload ->> 'fuel_economy', ''), '[^0-9.\\-]+', '', 'g'), '')::numeric
      else null
    end as consumption_kml,
    case
      when source_summary_report_id is not null
        and coalesce((last_summary_generated_at at time zone 'utc')::date, date '1900-01-01') = (now() at time zone 'utc')::date
        then coalesce(nullif(source_summary_payload ->> 'refill_count', '')::integer, 0)
      else 0
    end as refill_count,
    case
      when source_summary_report_id is not null
        and coalesce((last_summary_generated_at at time zone 'utc')::date, date '1900-01-01') = (now() at time zone 'utc')::date
        then coalesce(nullif(source_summary_payload ->> 'total_refill', '')::numeric, 0)
      else 0
    end as total_refill_volume,
    case
      when source_summary_report_id is not null
        and coalesce((last_summary_generated_at at time zone 'utc')::date, date '1900-01-01') = (now() at time zone 'utc')::date
        then coalesce(nullif(source_summary_payload ->> 'drain_count', '')::integer, 0)
      else 0
    end as drain_count,
    case
      when source_summary_report_id is not null
        and coalesce((last_summary_generated_at at time zone 'utc')::date, date '1900-01-01') = (now() at time zone 'utc')::date
        then coalesce(nullif(source_summary_payload ->> 'total_drain', '')::numeric, 0)
      else 0
    end as total_drain_volume
  from public.vehicles
)
update public.vehicles v
set
  total_distance = repaired.total_distance,
  total_engine_hours = repaired.total_engine_hours,
  total_fuel_used = repaired.total_fuel_used,
  consumption_kml = repaired.consumption_kml,
  refill_count = repaired.refill_count,
  total_refill_volume = repaired.total_refill_volume,
  drain_count = repaired.drain_count,
  total_drain_volume = repaired.total_drain_volume,
  updated_at = now()
from repaired
