...
function isRecoveredReplayRow(row: any): boolean {
  const lastError = String(row?.last_error ?? "").toLowerCase();
  return (
    lastError.includes("stale lock recovered") ||
    lastError.includes("worker fatal error recovered") ||
    lastError.includes("quarantined after repeated stale-lock stalls")
  );
}
...
async function coalescePendingReport0Rows(
  supabaseAdmin: any,
  options: {
    scanLimit: number;
    maxLoops: number;
  }
): Promise<Report0CoalesceSummary> {
  const scanLimit = Math.max(1, Math.trunc(options.scanLimit || 1));
  const maxLoops = Math.max(1, Math.trunc(options.maxLoops || 1));
  let deleted = 0;
  let loops = 0;
  ...
  try {
    for (let i = 0; i < maxLoops; i += 1) {
      const { data: rows, error: fetchError } = await supabaseAdmin
        .from("raw_telemetry_inbound")
        .select("id,source_imei,received_at")
        .eq("report_type", "0")
        .eq("status", "pending")
        .not("source_imei", "is", null)
      ...
      for (const row of rows ?? []) {
        const imei = normalizeImei(row?.source_imei);
      ...
      if (idsToDelete.length < scanLimit / 2) break;
    }
...
function roundNumeric(value: unknown, scale: number): number | null {
  const numeric = toNumeric(value);
  if (numeric === null) return null;
  return Number(numeric.toFixed(scale));
}

function sanitizeTripReportRow(row: any): any {
  const sanitized = { ...row };

  const registrationNumber = toOriginalText(sanitized.registration_number);
  if (registrationNumber) sanitized.registration_number = registrationNumber.slice(0, 30);

  const assetDescription = toOriginalText(sanitized.asset_description);
  if (assetDescription) sanitized.asset_description = assetDescription.slice(0, 100);

  const reportName = toOriginalText(sanitized.report_name);
  if (reportName) sanitized.report_name = reportName.slice(0, 255);

  sanitized.distance_km = roundNumeric(sanitized.distance_km, 6);
  sanitized.fuel_used_litres = roundNumeric(sanitized.fuel_used_litres, 3);
  sanitized.max_speed_kmh = toInteger(sanitized.max_speed_kmh);

  return sanitized;
}

async function upsertTripRowsOneByOne(
  supabaseAdmin: any,
  rows: any[],
  diagnostics?: ProcessingDiagnostics
): Promise<{ insertedCount: number; skippedOverflowCount: number }> {
  let insertedCount = 0;
  let skippedOverflowCount = 0;

  for (const row of rows) {
    const candidate = sanitizeTripReportRow(row);

    let { error: rowError } = await executeWithMissingColumnFallback(
      TRIP_REPORT_TABLE,
      candidate,
      async (candidatePayload) =>
    ...
    if (isNoUniqueConstraintError(rowError)) {
      try {
        await replaceTripRowsWithoutUniqueConstraint(supabaseAdmin, [candidate]);
        rowError = null;
      } catch (fallbackError) {
        rowError = fallbackError;
      }
    }

    if (rowError && String(rowError?.message ?? "").toLowerCase().includes("numeric field overflow")) {
      skippedOverflowCount += 1;
      noteDuplicateHandling(
        diagnostics,
        `trip_reports skipped overflow row ${toOriginalText(candidate.source_trip_key) ?? "unknown_trip_key"}`
      );
      continue;
    }
  ...
  return { insertedCount, skippedOverflowCount };
}
...
async function tryRecoverStaleLocks(
  supabaseAdmin: any,
  timeoutMinutes: number
): Promise<StaleLockRecoverySummary> {
  const summary: StaleLockRecoverySummary = {
    attempted: true,
    timeoutMinutes,
    recovered: 0,
    error: null
  ...
  try {
    const { data, error } = await supabaseAdmin.rpc("recover_stale_locks", {
    ...
    summary.recovered = toInteger(data) ?? 0;
    return summary;
  } catch (error: any) {
    summary.error = error?.message ?? String(error);
    return summary;
...
async function tryReleaseClaimedRows(
  supabaseAdmin: any,
  workerId: string,
  rowIds: string[],
  errorMessage: string
  ...
  try {
    const recoveredMessage = `Worker fatal error recovered: ${errorMessage.slice(0, 400)}`;
    const { data, error } = await supabaseAdmin
      .from("raw_telemetry_inbound")
      .update({
        status: "pending",
        last_error: recoveredMessage,
    ...
    if (error) {
      console.warn(`Claimed-row release skipped: ${error.message ?? error}`);
      return 0;
    }
...
async function chooseEffectiveBatchSize(
  supabaseAdmin: any,
  configuredBatchSize: number
): Promise<SerialReplayDecision> {
  const decision: SerialReplayDecision = {
    enabled: false,
    reason: null,
  ...
  try {
    ...
    const { data, error } = await supabaseAdmin
      .from("raw_telemetry_inbound")
      .select("id,last_error", { head: false })
      .eq("status", "pending")
      .lte("next_retry_at", nowIso)
      .or([
        "last_error.ilike.%Stale lock recovered%",
        "last_error.ilike.%Worker fatal error recovered%",
        "last_error.ilike.%Quarantined after repeated stale-lock stalls%"
      ].join(","))
      .limit(1);

    if (error) {
      console.warn(`Serial replay probe skipped: ${error.message ?? error}`);
      return decision;
    }

    if (Array.isArray(data) && data.length > 0) {
      decision.enabled = true;
      decision.reason = toCleanString(data[0]?.last_error) ?? "stale_lock_replay";
      decision.effectiveBatchSize = 1;
    }
...
async function upsertFuelNormalizedTables(
  ...
  const report50ReplaySensorSkipThreshold = Math.max(
    0,
    toInteger(Deno.env.get("REPORT50_REPLAY_SENSOR_SKIP_THRESHOLD") ?? "1000") ?? 1000
  );
  ...
  for (const entry of derivedRecords) {
    ...
    const shouldBypassBulkSql = isRecoveredReplayRow(row);

    if (telemetryRows.length > 0 || refills.length > 0 || drains.length > 0) {
      if (shouldBypassBulkSql) {
        const bypassReason = "recovered replay row";
        console.warn(`Bypassing report50 SQL bulk path for raw row ${row.id}: ${bypassReason}`);
      } else {
    ...
    if (!usedBulkSql) {
      ...
      const shouldSkipReplaySensorRows =
        isRecoveredReplayRow(row) && telemetryRows.length > report50ReplaySensorSkipThreshold;

      if (shouldSkipReplaySensorRows) {
        console.warn(
          `Skipping raw_sensor_data detail rows for replay raw row ${row.id}: telemetry count ${telemetryRows.length} exceeds replay threshold ${report50ReplaySensorSkipThreshold}`
        );
      } else {
...
async function upsertDailyMovementFromTripReport(
  ...
  if (upsertError && String(upsertError?.message ?? "").toLowerCase().includes("numeric field overflow")) {
    const fallback = await upsertTripRowsOneByOne(supabaseAdmin, deduped, diagnostics);
    if (fallback.insertedCount > 0) {
      noteDuplicateHandling(
        diagnostics,
        `trip_reports row-by-row fallback inserted ${fallback.insertedCount} and skipped ${fallback.skippedOverflowCount} overflow rows`
      );
      return {
        count: fallback.insertedCount,
        firstImei: deduped.find((record) => normalizeImei(record?.source_imei))?.source_imei ?? null,
        firstReportName: reportName,
        skippedOverflowCount: fallback.skippedOverflowCount
      };
    }
  }
...
serve(async (req: Request) => {
  ...
  const staleLockTimeoutMinutes = Math.max(1, toInteger(Deno.env.get("STALE_LOCK_TIMEOUT_MINUTES") ?? "10") ?? 10);
  ...
  const report0CoalesceScanLimit = Math.max(1, toInteger(Deno.env.get("REPORT0_COALESCE_SCAN_LIMIT") ?? "1000") ?? 1000);
  const report0CoalesceMaxLoops = Math.max(1, toInteger(Deno.env.get("REPORT0_COALESCE_MAX_LOOPS") ?? "25") ?? 25);
  ...
  let claimedRowIds: string[] = [];

  try {
    const staleLockRecovery = await tryRecoverStaleLocks(supabaseAdmin, staleLockTimeoutMinutes);
    if (staleLockRecovery.error) {
      console.error("Stale-lock recovery failed:", staleLockRecovery.error);
    } else if (staleLockRecovery.recovered > 0) {
      console.log(
        `Recovered ${staleLockRecovery.recovered} stale processing rows older than ${staleLockRecovery.timeoutMinutes} minute(s)`
      );
    }

    const serialReplayDecision = await chooseEffectiveBatchSize(supabaseAdmin, batchSize);
    if (serialReplayDecision.enabled) {
      console.log(
        `Serial replay mode enabled for recovered/quarantined rows (effective batch size ${serialReplayDecision.effectiveBatchSize})`
      );
    }
    ...
    const coalesceSummary = await coalescePendingReport0Rows(supabaseAdmin, {
      scanLimit: report0CoalesceScanLimit,
      maxLoops: report0CoalesceMaxLoops
    });
    if (coalesceSummary.error) {
      console.error("Report0 coalesce failed:", coalesceSummary.error);
    } else if (coalesceSummary.deleted > 0) {
      console.log(
        `Report0 coalesce deleted ${coalesceSummary.deleted} superseded pending rows (loops=${coalesceSummary.loops})`
      );
    }

    const { data: batch, error: claimError } = await supabaseAdmin.rpc("claim_ingestion_batch", {
      p_worker_id: workerId,
      p_batch_size: serialReplayDecision.effectiveBatchSize
    });
    ...
    if (!batch || batch.length === 0) {
      ...
      return new Response(
        JSON.stringify({
          ...
          run_id: runId,
          serial_replay: serialReplayDecision,
          stale_lock_recovery: staleLockRecovery,
          report0_purge: purgeSummary,
          report0_coalesce: coalesceSummary
        }),
        {
          headers: { "content-type": "application/json" }
        }
    ...
    claimedRowIds = batch.map((row: any) => String(row?.id ?? "")).filter(Boolean);
    ...
    const releasedCount = await tryReleaseClaimedRows(
      supabaseAdmin,
      workerId,
      claimedRowIds,
    ...
    if (releasedCount > 0) {
      console.log(`Released ${releasedCount} claimed rows after fatal worker error`);
    }
    await tryFinalizeProcessingRunLog(supabaseAdmin, runId, {
      status: "fatal",
      claimed: claimedRowIds.length,
      processed: 0,
      failed: 0,
      skipped: 0,
      error: String(err?.message ?? err)
