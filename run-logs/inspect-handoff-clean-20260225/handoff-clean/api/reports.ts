...
function parseAssetId(value: any): string | undefined {
...
class DailyMovementGenerator {
  ...
  groupByAssetForPreview(data: any[]) {
    ...
    for (const item of data) {
      if (!item.is_total_row && !item.is_average_row) {
        const key = item.registration_number || item.asset_description || item.asset_id || item.vehicle_id || item.id;
  ...
  sumTimes(timeStrings: string[]) {
    ...
    for (const timeStr of timeStrings) {
      totalSeconds += this.parseDurationToSeconds(timeStr);
    }
  ...
  averageTimes(timeStrings: string[]) {
    const validTimes = timeStrings
      .map((value) => this.parseDurationToSeconds(value))
      .filter((seconds) => seconds > 0);
    if (validTimes.length === 0) return '00:00:00';

    const totalSeconds = validTimes.reduce((sum, seconds) => sum + seconds, 0);
  ...
  parseDurationToSeconds(value: string | null | undefined) {
    if (!value) return 0;
  ...
  formatStandingTimeForExcel(standingTime: string | null | undefined): string {
    if (!standingTime) return '';
  ...
  async getMovementData(startDate: string, filters: DailyMovementFilters = {}, endDate?: string) {
    ...
    if (filters.vehicleId && rows.length === 0 && (filters.assetId !== undefined || filters.registrationNumber)) {
      ...
      if (filters.assetId !== undefined) {
        fallbackQuery = fallbackQuery.eq("source_imei", filters.assetId);
      } else if (filters.registrationNumber) {
    ...
    return rows.map((row: any) => ({
      ...
      asset_id: row.source_imei || row.source_assigned_asset || row.vehicle_id,
      site_name: "",
      ...
      is_average_row: row.is_average_row,
    }));
  }
}
  ...
  async getFuelTemperatureDataRange(
    ...
    const [
      ...
      supabaseAdmin
        .from("fuel_events")
        .select("*")
        .in("temperature_report_id", reportIds)
    ...
    (refuelEvents || []).forEach((row: any) => {
      const reportId = row.temperature_report_id;
