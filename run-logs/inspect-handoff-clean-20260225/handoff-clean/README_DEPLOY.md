...
- Saves the full value to `daily_movement_reports.report_name`.
- Saves extracted parts to:
  - `daily_movement_reports.source_imei`
  - `daily_movement_reports.client_name`
  - `daily_movement_reports.source_assigned_asset`
...
- Each trip row is written to `daily_movement_reports` (not collapsed into one row).
