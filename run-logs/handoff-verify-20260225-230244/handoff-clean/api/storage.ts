...
const mapVehicleInsert = (vehicle: InsertVehicle) => ({
  ...
  asset_id: vehicle.assetId || (vehicle as any).imei || vehicle.vehiclePlate,
...
export class SupabaseStorage implements IStorage {
  ...
  async createUserSettings(settings: InsertUserSettings): Promise<UserSettings> {
    const payload = cleanPayload(mapUserSettingsInsert(settings));
    const { data, error } = await supabaseAdmin
      .from("user_settings")
      .insert([payload])
      .select("*")
  ...
  async updateUserSettings(updates: UpdateUserSettings): Promise<UserSettings | undefined> {
    const payload = cleanPayload(mapUserSettingsInsert(updates as InsertUserSettings));
    const { data, error } = await supabaseAdmin
      .from("user_settings")
      .update(payload)
